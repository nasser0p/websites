...
const TestimonialCard: React.FC<{ testimonial: Review, text: TextContent, isVisible: boolean, index: number }> = ({ testimonial, text, isVisible, index }) => {
    ...
    return (
        <div 
            ...
            style={{ ...style, animationDelay: `${300 + index * 100}ms` }}
            className={`bg-gray-50 p-8 rounded-lg shadow-lg relative flex flex-col tilt-card hover:shadow-2xl hover:shadow-brand-teal/20 tappable ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}
        >
    ...
    return (
        <section id="reviews" className="bg-white py-20 md:py-28" ref={ref}>
            ...
            <div className="container mx-auto px-6">
                ...
                <div 
                    className={`mt-12 max-w-md mx-auto bg-gray-50 p-6 rounded-lg shadow-lg text-center transition-transform duration-300 hover:-translate-y-2 hover:shadow-xl hover:shadow-brand-teal/20 tappable ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}
