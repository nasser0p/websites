...
const StaffCard: React.FC<{ staffMember: StaffMember, text: TextContent, isVisible: boolean, index: number }> = ({ staffMember, text, isVisible, index }) => {
    ...
    return (
        <div 
            ...
            className={`bg-white p-8 rounded-lg shadow-xl flex flex-col sm:flex-row items-center gap-8 text-center sm:text-left rtl:sm:text-right tilt-card hover:shadow-2xl hover:shadow-brand-teal/20 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
        >
...
const About: React.FC<AboutProps> = ({ staff }) => {
  ...
  return (
    <section id="about" className="py-20 md:py-28 bg-gray-50 section-divider-bottom" ref={ref}>
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className={`mt-16 max-w-4xl mx-auto bg-white p-10 rounded-lg shadow-xl text-center transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '200ms' }}>
