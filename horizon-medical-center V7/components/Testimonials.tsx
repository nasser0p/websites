...
const TestimonialCard: React.FC<{ testimonial: Review, text: TextContent, isVisible: boolean, index: number }> = ({ testimonial, text, isVisible, index }) => {
    ...
    return (
        <div 
            ...
            className={`bg-gray-50 p-8 rounded-lg shadow-lg relative flex flex-col tilt-card hover:shadow-2xl hover:shadow-brand-teal/20 tappable transition-all duration-700 ease-in-out ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-5 scale-95'}`}
        >
            ...
            <figcaption className="mt-6 pt-6 border-t border-gray-200 flex items-center gap-4">
               <img src={testimonial.avatarUrl} alt={testimonial.name} className="h-12 w-12 rounded-full object-cover shadow-md" />
...
const Testimonials: React.FC<TestimonialsProps> = ({ testimonials, googleReviews }) => {
    ...
    return (
        <section id="reviews" className="bg-white py-20 md:py-28" ref={ref}>
            ...
            <div className="container mx-auto px-6">
                ...
                <div 
                    className={`mt-12 max-w-md mx-auto bg-gray-50 p-6 rounded-lg shadow-lg text-center transition-all duration-700 ease-in-out hover:-translate-y-2 hover:shadow-xl hover:shadow-brand-teal/20 tappable ${isVisible ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-5 scale-95'}`}
